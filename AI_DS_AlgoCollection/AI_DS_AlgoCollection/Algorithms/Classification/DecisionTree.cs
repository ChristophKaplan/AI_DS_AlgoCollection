...
public abstract class DtNode<TDataType, TDataValue> where TDataType : notnull
{
    ...
    private DtEdge GetIncomingEdge()
    {
        return IsRoot() ? null : Parent.GetOutgoingEdge(this);
    }

    private DtEdge GetOutgoingEdge(DtNode<TDataType, TDataValue> node)
    {
        return _edges.FirstOrDefault(edge => edge.GetNode().Equals(node));
    }
    ...
    private class DtEdge
    {
        ...
        public DtNode<TDataType, TDataValue> GetNode()
        {
            return _node;
        }
    }
...
public class DtNodeAttribute<TDataType, TDataValue> : DtNode<TDataType, TDataValue> where TDataType : notnull
{
    ...
    public DtNodeAttribute(TDataType attribute, DtNode<TDataType, TDataValue> parent,
        DataTable<TDataType, TDataValue> data) : base(parent, data)
    {
...
public class DtNodeClassification<TDataType, TDataValue> : DtNode<TDataType, TDataValue> where TDataType : notnull
{
    ...
    public DtNodeClassification(bool truthValue, DtNode<TDataType, TDataValue> parent,
        DataTable<TDataType, TDataValue> data) : base(parent, data)
    {
...
public class DecisionTree<TDataType, TDataValue> where TDataType : notnull
{
    ...
    private DtNode<TDataType, TDataValue> ConstructDecisionTree(ClassifictaionData<TDataType, TDataValue> dtData,
        bool defaultVal, DtNode<TDataType, TDataValue> parent = null)
    {
        //Console.WriteLine(data.PrintInfo());

        if (dtData.RowCount == 0) return new DtNodeClassification<TDataType, TDataValue>(defaultVal, parent, dtData);
        if (dtData.IsSameClassificationForExamples())
            return new DtNodeClassification<TDataType, TDataValue>(dtData.Classification[0], parent, dtData);
...
{
    public static DecisionTree<TDataType, TDataValue> GetDT<TDataType, TDataValue>(
        this ClassifictaionData<TDataType, TDataValue> data) where TDataType : notnull
    {
        return new DecisionTree<TDataType, TDataValue>(data);
    }

    public static TDataType ChooseAttribute<TDataType, TDataValue>(this ClassifictaionData<TDataType, TDataValue> data,
        bool id3 = false)
    {
    ...
    private static float InformationEntropy<TDataType, TDataValue>(this ClassifictaionData<TDataType, TDataValue> data)
    {
        ...
        var probabilityToPickAPositiveExample = data.GetPosOrNegExamples().Count / exampleCount;
        return Entropy(probabilityToPickAPositiveExample, probabilityToPickANegativeExample);
    }

    private static float ConditionalEntropy<TDataType, TDataValue>(this ClassifictaionData<TDataType, TDataValue> data,
        TDataType type)
    {
    ...
    private static float InformationGain<TDataType, TDataValue>(this ClassifictaionData<TDataType, TDataValue> data,
        TDataType attribute)
    {
        return data.InformationEntropy() - data.ConditionalEntropy(attribute);
    }

    private static float GainRatio<TDataType, TDataValue>(this ClassifictaionData<TDataType, TDataValue> data,
        TDataType attribute)
    {
        var gain = data.InformationGain(attribute);
        var splitInfo = data.SplitInfo(attribute);
        if (splitInfo == 0) return 0f;

        return gain / splitInfo;
    }

    private static float SplitInfo<TDataType, TDataValue>(this ClassifictaionData<TDataType, TDataValue> data,
        TDataType attribute)
    {
    ...
    private static (TDataType, float) ID3_GetMaxGain<TDataType, TDataValue>(
        this ClassifictaionData<TDataType, TDataValue> data)
    {
    ...
    private static (TDataType, float) C4_5_GetMaxGainRatio<TDataType, TDataValue>(
        this ClassifictaionData<TDataType, TDataValue> data)
    {
        (TDataType, float) max = default;
        ...
        foreach (var cls in data.Types)
        {
            var currentGain = data.GainRatio(cls);
            if (currentGain > max.Item2) max = (cls, currentGain);
        }
