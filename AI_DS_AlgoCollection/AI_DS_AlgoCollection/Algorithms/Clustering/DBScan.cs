...
public static class DbScan
{
    ...
    private static Dictionary<TDataValue, string> DbScanAlgorithm<TDataValue>(List<TDataValue> data, int minPts,
        float epsilon, Func<TDataValue, TDataValue, float> distanceFunc)
    {
        var id = 0;
        ...
        foreach (var dataPoint in data)
            if (result[dataPoint] == Undefined)
            {
                ...
                else
                {
                    ...
                    {
                        var current = set[i];
                        if (result[current] == Noise) result[current] = id.ToString(); //border point
                        ...
                        if (epsNeighborhood.Count >= minPts) set.AddRange(epsNeighborhood);
                    }
                }
            }
    ...
    private static List<TDataValue> RangeQuery<TDataValue>(TDataValue dataPoint, List<TDataValue> data, float epsilon,
        Func<TDataValue, TDataValue, float> distanceFunc)
    {
