...
{
    public static void DoApriori<TDataType, TDataValue>(this EventData<TDataType, TDataValue> data, int minNum = 3)
        where TDataType : IComparable
    {
        ...
        foreach (var itemSets in aprioriResult) result.AddRange(itemSets);

        result = result.SortingBy(itemSetsOrigin);

        Console.WriteLine(result.Aggregate("Apriori: \t",
            (current, itemSet) => current + itemSet + itemSetsOrigin.Num(itemSet) + ", "));
        var b = result.CreateRulesFromItemSets(0.3f, itemSetsOrigin);
    }

    private static List<List<ItemSet<TDataType>>> AprioriAlgo<TDataType>(this List<ItemSet<TDataType>> itemSets,
        int minNum) where TDataType : IComparable
    {
    ...
    private static Dictionary<ItemSet<TDataType>, int> FrequencyCount<TDataType>(List<ItemSet<TDataType>> itemSets,
        List<ItemSet<TDataType>> candidates)
    {
        ...
        foreach (var candidate in candidates)
        {
            ...
            if (itemSet.Contains(candidate)) frequencyCount[candidate]++;
        }

        return frequencyCount;
    }

    internal static List<ItemSet<TDataType>> AprioriGen<TDataType>(this List<ItemSet<TDataType>> itemSets)
        where TDataType : IComparable
    {
        ...
        foreach (var p in itemSets)
        {
            ...
            foreach (var q in itemSets)
            {
                ...
                if (!candidates.Contains(newSet)) candidates.Add(newSet);
            }
        }
    ...
    private static ItemSet<TDataType> KeepItemsUntilK_JoinLastTwo<TDataType>(ItemSet<TDataType> p, ItemSet<TDataType> q,
        int k)
    {
        var joined = new List<TDataType>();
        for (var i = 0; i < k; i++) joined.Add(p.ItemList[i]);
