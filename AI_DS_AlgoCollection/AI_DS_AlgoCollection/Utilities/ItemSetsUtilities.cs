...
public static class ItemSetsUtilities
{
    public static Dictionary<ItemSet<TDataType>, List<int>> VerticalFormat<TDataType>(
        this List<ItemSet<TDataType>> itemSets)
    {
    ...
    private static List<int> IndicesOfSetsThatContainItem<TDataType>(this List<ItemSet<TDataType>> itemSets,
        ItemSet<TDataType> itemSet)
    {
    ...
    internal static List<ItemSet<TDataType>> GetFrequentSingleItemSetsByMinNum<TDataType>(
        this List<ItemSet<TDataType>> itemSets, float minNum)
    ...
    public static float Support<TDataType>(this List<ItemSet<TDataType>> itemSets, ItemSet<TDataType> itemSet)
    {
        return itemSets.Num(itemSet) / (float)itemSets.Count;
    }

    public static float Support<TDataType>(this List<ItemSet<TDataType>> itemSets, AssociationRule<TDataType> rule)
    {
        return itemSets.Num(rule.GetJoined()) / (float)itemSets.Count;
    }

    public static float Confidence<TDataType>(this List<ItemSet<TDataType>> itemSets, AssociationRule<TDataType> rule)
    {
        return itemSets.Support(rule) / itemSets.Support(rule.X);
    }
    ...
    public static List<ItemSet<TDataType>> SortingBy<TDataType>(this List<ItemSet<TDataType>> itemSets,
        List<ItemSet<TDataType>> sortBy)
    {
        foreach (var itemSet in itemSets) itemSet.ItemList = itemSet.ItemList.OrderByDescending(sortBy.Num).ToList();
